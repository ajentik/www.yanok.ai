...
export default function Page() {
  ...
  <div class="flex flex-row gap-12">
    <img class="size-20" src="/assets/images/integrations/baserow.png" />
    <div class="flex items-center justify-between grow">
      <h1>
        Baserow Integrations
        <span class="block max-w-full h-0.5" style="background-color: rgba(25, 141, 214, 1)"></span>
      </h1>
    </div>
  </div>
  <p class="max-w-3xl mt-8 text-center">Baserow is an open source no-code database tool and Airtable alternative.</p>
  ...
  <div>Trigger when new row is created.</div>
</div>

<div class="flex flex-col items-start">
  <div class="font-semibold">Row Created or Updated</div>
  <div>Trigger when a new row is created or an existing one is updated.</div>
</div>

<div class="flex flex-col items-start">
  <div class="font-semibold">Row Updated</div>
  <div>Trigger when an existing row is updated.</div>
</div>
  ...
  return {
    props: {
      metaTags: (
        <>
          <title>Baserow</title>
          <meta name="description" content={`Baserow is an open source no-code database tool and Airtable alternative.`} />
        </>
      ),
    },
  };
